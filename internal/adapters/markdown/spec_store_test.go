...
func TestRoundTripPreservesLiterateSpecFields(t *testing.T) {
	...
	model.Review = spec.ReviewState{Status: "completed", Verdict: corereview.VerdictFail, Mode: corereview.ModeDiscover, Provider: "claude", Model: "claude-test", OutputFormat: "claude.structured_output", Normalizations: []string{"finding.location_string"}, Summary: "Review found an open blocker.", Findings: []corereview.Finding{{ID: "f1", Severity: corereview.SeverityHigh, BlocksCompletion: true, Location: &corereview.Location{Path: "file.go"}, Evidence: "bug", Impact: "test impact", Validation: "rerun test", Summary: "bug"}}}
	...
	if parsed.Review.Provider != "claude" || parsed.Review.Model != "claude-test" || parsed.Review.OutputFormat != "claude.structured_output" || len(parsed.Review.Normalizations) != 1 || parsed.Review.Normalizations[0] != "finding.location_string" {
