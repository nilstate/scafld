...
func PrintCommand(w io.Writer, name string, commands []Command) {
	...
	if name == "build" {
		fmt.Fprint(w, `scafld build - Open or advance governed build phases

Usage:
  scafld build <task_id> [--root PATH] [--json]
  scafld build <task_id> --criterion ID --disposition pass|fail --evidence-digest SHA256 --actor ACTOR --reason TEXT [--root PATH] [--json]

Without manual-evidence flags, opens or advances the current acceptance phase.
For a manual acceptance criterion, provide the verified disposition on this
same build command. scafld appends the typed evidence, evaluates acceptance,
and prints the next lifecycle command. The timestamp is generated by scafld;
the source Markdown is never edited by this path.

Flags:
  --criterion ID          Manual acceptance criterion id
  --disposition VALUE     pass or fail
  --evidence-digest HEX   SHA-256 digest of the verified evidence
  --actor ACTOR           Human or system actor recording the disposition
  --reason TEXT            What the evidence established
  --root PATH              Workspace root
  --json                   Print JSON envelope
`)
		return
