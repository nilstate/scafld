...
func TestReceiptGradeClaudeSandboxMemoryReadRoot(t *testing.T) {
	...
	for _, want := range []string{"--no-session-persistence", "--disable-slash-commands", "--disallowedTools", "Agent,Task,Bash,Edit,MultiEdit,Write,NotebookEdit", "--add-dir", claude.CWD} {
		if !containsArg(args, want) {
