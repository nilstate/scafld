...
func reviewOutputContractSection() reviewcontext.Section {
	body := "Use exactly one provider submission transport. If the `" + review.SubmitToolName + "` tool is exposed, call it exactly once with the final ReviewDossier; otherwise return exactly one JSON object matching the ReviewDossier schema. MCP and Codex expose the same strict wire schema. Every declared property is required on the wire; use null for semantically unused nullable fields. Never emit prose, markdown, or a second JSON value. `verdict` must be `pass` or `fail`; `mode` must be `discover` or `verify`; `summary` must explain the review result. `findings` must be an array of typed defect objects. Findings are defects only, blocking or not; improvements, nice-to-have refactors, preferences, marginal-surface enumeration, and per-consumer bookkeeping are not findings unless they identify a verified defect, violated shared invariant, or broken adapter boundary. Use severity `critical`, `high`, `medium`, or `low`, then set `blocks_completion` true only when completion must stop. Completion-blocking findings require `location`, `evidence`, `impact`, `suggested_fix`, and `validation`. Final-shape drift findings are repair packets: include `suggested_fix` and `validation` that tell the executor exactly what to clean up and how to prove it. `attack_log` must record the bounded attacks you actually performed, using result `finding`, `clean`, or `skipped`. Clean attack log entries must name the concrete target inspected and why the attack held."
