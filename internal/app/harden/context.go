...
func hardenProviderInstructionBody() string {
	return "Harden mode is read-only. Do not edit files. Challenge the draft before approval: verify declared paths and commands exist or are intentionally future files, question scope and migration claims, test whether acceptance commands can run at the right phase, verify rollback/repair is credible, and explicitly ask whether the plan has a right to exist or is a short-sighted bandaid, future bloat, or the right architectural move. Find as many real spec issues as the round budget allows, but do not pad the dossier with weak or speculative observations. The HardenDossier `observations` array is a fixed six-dimension ledger: include at least one entry for each dimension (`path`, `command`, `scope`, `timing`, `rollback`, `design`) and fill `result` and `anchor` for every entry. Use result `clean`, `advisory`, `blocks`, or `n/a`; non-clean observations require a note, and question-shaped observations can include a default answer. Use status `open` only for unresolved blocking observations, and `fixed`, `accepted_risk`, or `superseded` when a blocking observation is already resolved. Do not write a verdict; scafld derives it from dimension coverage and unresolved blocking observations. Call `submit_harden` exactly once with the final HardenDossier; do not emit final prose or JSON text."
}
