// Package gate owns the host-facing accountability gate use case.
package gate
...
// Snapshotter computes the commit-free tree facts reviewed by the gate.
...
func Run(ctx context.Context, snapshotter Snapshotter, acceptanceRunner AcceptanceRunner, reviewer Reviewer, signer Signer, input Input) (Output, error) {
	...
	if !accepted.Passed {
		return Output{Verdict: review.VerdictFail, Acceptance: accepted, Reason: "acceptance failed before review"}, nil
	}
	...
	if post.TreeSHA != snap.TreeSHA {
		return Output{Verdict: review.VerdictFail, Acceptance: accepted, Reason: "workspace mutated during acceptance; gate failed closed"}, nil
	}
	...
	if len(blockers) > 0 {
		return Output{Verdict: review.VerdictFail, Acceptance: accepted, Findings: findings, Reason: "review blockers remain"}, nil
	}
	...
	return Output{Verdict: review.VerdictPass, Acceptance: accepted, Findings: findings, Receipt: &envelope}, nil
}
