...
Latest runner update: 2026-06-04T08:28:56Z
Review gate: not_started

## Summary

Add `scafld verify`, the merge-time check that independently re-derives every claim in a signed receipt from the checked-out PR head and an explicit merge target. It needs only the checked-out tree, the committed task receipt, a trusted-keys allowlist loaded from a protected source, and a CI-provided `--target <commit-ish>`; no provider call or hosted infrastructure. The check recomputes `tree_sha` and `base_commit` from the committed tree, re-runs the receipt's acceptance commands and confirms the exit codes match, verifies the ed25519 signature against a non-revoked trusted public key, and asserts the receipt's verdict, independence policy, coverage, and ancestry invariants before exiting zero. Any divergence between the recomputed reality and the receipt exits nonzero so branch protection can block the merge.
...
- Verify the detached ed25519 signature over the canonical receipt body against a trusted key allowlist parsed by `internal/core/trust`, rejecting revoked or absent `key_id`s. In CI, the allowlist path must be supplied by `--trusted-keys` or `SCAFLD_TRUSTED_KEYS`, normally extracted from the protected target commit; CI must not trust a PR-controlled config default.
...
- In scope: ed25519 verification against a trusted `trusted-keys.json` allowlist file; reuse spec 4's canonical-body/signature types and `internal/core/trust` trusted-key parser/key-id validation.
...
- `internal/core/review/model.go` already enumerates providers in `ValidCompletionProvider` (`codex`, `claude`, `gemini`, `command`, `human`); this finalize rejects `command` and `human` on the verify path.
...
- The task receipt is committed in the PR head at a known path (default `.scafld/receipts/<task_id>.json`). `latest.json` is local convenience and must not be the only CI artifact.
- In CI, trusted keys come from a protected source, normally `git show <target>:.scafld/trusted-keys.json` written to `$RUNNER_TEMP` and passed as `--trusted-keys`; PR-controlled `.scafld/config.yaml` or `.scafld/trusted-keys.json` cannot select the trust anchor.
...
- `Snapshotter`: recomputes `tree_sha`, `base_commit`, file digests, ignored paths, and coverage facts for the checked-out tree.
...
`internal/core/trust` is the single owner of `.scafld/trusted-keys.json`: `TrustedKeys`, `TrustedKey`, parser/serializer, `KeyIDFromRawEd25519PublicKey`, duplicate detection, revocation checks, and key-id/public-key consistency. Verify reads the allowlist through this package and rejects unknown ids, duplicate ids, non-ed25519 keys, invalid base64, mismatched `key_id`, and keys with non-null `revoked_at`. Init writes through the same package; verify must not define a parallel trust schema. CI passes a protected allowlist path explicitly instead of relying on PR-controlled repo config.
...
- `.github/actions/scafld-verify/action.yml` (new) and `scripts/scafld-verify.sh` (new): the pre-merge wrapper that loads protected trusted keys, chooses one committed task receipt, installs a pinned scafld version when needed, and exits nonzero on failure.
- `.scafld/trusted-keys.json` (read-only here; created by spec 6 through `internal/core/trust`, but loaded from the protected target in CI).

## Risks

- Acceptance commands that are nondeterministic or environment-sensitive could fail re-run in CI even when the receipt is honest; mitigated by applying spec 1's env-scrub allowlist so CI matches the finalize runtime, and by surfacing the exact diverging command in the failure reason.
...
- `internal/app/verify/verify_test.go` - Table tests covering pass and each failure mode: tree mismatch, base_commit mismatch, revoked key, unknown key, duplicate key from `internal/core/trust`, mismatched key_id from `internal/core/trust`, non-pass verdict, nonzero open_blockers, dirty mutation_guard, same-vendor reviewer when policy is `cross_vendor`, below-minimum independence, missing in-scope digest, non-ancestor base_commit, missing target in CI policy, and `command`/`human` provider.
...
Objective: Re-run the receipt's acceptance commands through an `AcceptanceRunner` implementation backed by the shared acceptance/process runner and confirm each observed exit code matches the recorded `exit_code` and `status`, applying spec 1's env contract so CI reproduces the finalize runtime rather than the host environment. Wire a `scafld verify <receipt-path> --target <commit-ish> --trusted-keys <path>` cli subcommand that loads the receipt and trusted keys, reads `verify.min_independence` and non-CI defaults from config, requires `--target` and an explicit trusted-key path in CI, invokes the Phase 1 use case plus this acceptance re-run, and exits nonzero with the failing invariant's reason on any failure. Composition lives in the `internal/adapters/cli/verify` subpackage; the root handler registered in `cli.go` stays thin per `TestCLIIsThin`, while subpackage discipline is covered by app boundary and port tests.
...
Objective: Ship a GitHub Action and pre-merge wrapper script that checks out the PR head, resolves the merge target commit/ref, loads the verify wrapper and `trusted-keys.json` from the protected target, invokes `scafld verify <receipt-path> --target <commit-ish> --trusted-keys <protected-path>` against one committed task receipt, and propagates the exit code so branch protection blocks the merge on any failure. The wrapper adds no verification logic of its own; it only locates the receipt path, target, protected trust anchor, and pinned scafld binary before forwarding to `scafld verify`, keeping the single source of truth in the use case.
...
- Resolved: CI verifies exactly one committed task receipt unless explicitly configured, keeps `latest.json` as local convenience only, and loads `trusted-keys.json` from the protected target before passing `--trusted-keys`.
...
Summary: Needs revision: the merge finalize is worth building, but approval should wait until the spec fixes the app/adapters boundary, independence contradiction, target-ref input, trusted-keys schema, and phase ownership for acceptance re-runs.
...
- design challenge
  ...
  - Result: failed
  - Evidence: The merge finalize is the right architectural move, but the draft currently contradicts the independence model and lacks executable trust-key and CI target-ref contracts.
