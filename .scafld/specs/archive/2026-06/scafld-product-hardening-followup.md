...
Latest runner update: 2026-06-11T03:31:18Z
Review gate: pass
...
Validation:
- [x] `v1` test - Full check passes.
  - Command: `make check`
  - Expected kind: `exit_code_zero`
  - Status: pass
  - Evidence: exit code was 0
  - Source event: entry-146
- [x] `v2` release - Snapshot release artifacts build.
  - Command: `make release-snapshot`
  - Expected kind: `exit_code_zero`
  - Status: pass
  - Evidence: exit code was 0
  - Source event: entry-147
- [x] `v3` boundary - Legacy workspace and YAML task-spec logic are absent from production code and docs.
  - Command: `rg -n '\\.ai/|\\.ai/config|\\.yaml task spec|YAML task spec|legacy Python runtime|python runtime fallback' README.md docs internal cmd package .scafld/core --glob '!*_test.go'`
  - Expected kind: `no_matches`
  - Status: pass
  - Evidence: output was empty
  - Source event: entry-148
...
- [x] `v5` tool - At least one external challenger is installed for real review before release.
  - Command: `command -v codex >/dev/null || command -v claude >/dev/null`
  - Expected kind: `exit_code_zero`
  - Status: pass
  - Evidence: exit code was 0
...
- [x] `ac3_3` boundary - Review sealing fields are present in code or schema.
...
- [x] `ac3_4` boundary - Complete cannot rely only on spec text for review success.
...
Acceptance:
- [x] `ac4_1` test - Process runner timeout, capture, and signal tests pass under race.
  - Command: `go test -race ./internal/adapters/process ./internal/platform/signal`
  - Expected kind: `exit_code_zero`
  - Status: pass
  - Evidence: exit code was 0
  - Source event: entry-52
- [x] `ac4_2` test - Provider adapters and provider fakes cover all failure modes.
  - Command: `go test ./internal/adapters/providers ./internal/testkit/providerfake ./internal/app/review`
  - Expected kind: `exit_code_zero`
  - Status: pass
  - Evidence: exit code was 0
  - Source event: entry-53
- [x] `ac4_3` boundary - Provider diagnostics include kill reason and timeout context.
...
- [x] `ac4_4` test - Mutation guard detects content modifications, additions, deletions, and untracked files.
...
Acceptance:
- [x] `ac5_1` release - Snapshot artifacts build for supported platforms.
  - Command: `make release-snapshot`
  - Expected kind: `exit_code_zero`
  - Status: pass
  - Evidence: exit code was 0
  - Source event: entry-68
- [x] `ac5_2` test - Release package tests pass.
  - Command: `go test ./test/release ./test/ci`
  - Expected kind: `exit_code_zero`
  - Status: pass
  - Evidence: exit code was 0
  - Source event: entry-69
- [x] `ac5_3` npm - npm wrapper packs cleanly.
  - Command: `cd package/npm && npm pack --dry-run`
  - Expected kind: `exit_code_zero`
  - Status: pass
  - Evidence: exit code was 0
  - Source event: entry-70
- [x] `ac5_4` pypi - PyPI wrapper builds cleanly in an isolated environment.
  - Command: `python3 -m venv --clear --upgrade-deps /tmp/scafld-pypi-build && /tmp/scafld-pypi-build/bin/python -m pip install build >/dev/null && /tmp/scafld-pypi-build/bin/python -m build package/pypi`
  - Expected kind: `exit_code_zero`
  - Status: pass
  - Evidence: exit code was 0
  - Source event: entry-71
- [x] `ac5_5` docs - Release docs name each supported registry and the verification command for that registry.
  - Command: `for pattern in 'GitHub release' 'npm install -g scafld@X.Y.Z' 'pipx install scafld==X.Y.Z' 'go install github.com/nilstate/scafld/v2/cmd/scafld@vX.Y.Z' 'Homebrew' 'Scoop' 'WinGet' 'docker manifest inspect ghcr.io/nilstate/scafld:vX.Y.Z'; do rg -n "$pattern" docs/release.md docs/installation.md >/dev/null || exit 1; done`
  - Expected kind: `exit_code_zero`
  - Status: pass
  - Evidence: exit code was 0
  - Source event: entry-72
...
Acceptance:
- [x] `ac6_1` test - Report and session replay tests pass.
  - Command: `go test ./internal/app/report ./internal/core/session ./internal/core/reconcile`
  - Expected kind: `exit_code_zero`
  - Status: pass
  - Evidence: exit code was 0
  - Source event: entry-77
- [x] `ac6_2` smoke - Report command works in an initialized workspace.
  - Command: `go test ./test/e2e -run Report`
  - Expected kind: `exit_code_zero`
  - Status: pass
  - Evidence: exit code was 0
  - Source event: entry-78
- [x] `ac6_3` boundary - README metrics claims are session-derived and not model-magic claims.
...
Acceptance:
- [x] `ac7_1` docs - Documentation parity and golden tests pass.
  - Command: `go test ./test/parity ./internal/testkit/golden ./internal/adapters/markdown`
  - Expected kind: `exit_code_zero`
  - Status: pass
  - Evidence: exit code was 0
  - Source event: entry-91
- [x] `ac7_2` boundary - Docs do not mention old sentinel behavior, YAML task specs, or Python runtime fallback.
  - Command: `rg -n 'sentinel marker|scafld sentinel|YAML task spec|\\.yaml spec|Python runtime fallback|legacy Python runtime|\\.ai/' README.md docs .scafld/core .scafld/prompts`
  - Expected kind: `no_matches`
  - Status: pass
  - Evidence: output was empty
  - Source event: entry-92
- [x] `ac7_3` docs - README clearly explains why harden matters and why review is adversarial.
  - Command: `for pattern in 'Hardening lives between draft and approval' 'discipline of attacking the spec' 'agent does not get to grade its own homework' 'separate model review' 'blocking issue'; do rg -n "$pattern" README.md >/dev/null || exit 1; done`
  - Expected kind: `exit_code_zero`
  - Status: pass
  - Evidence: exit code was 0
  - Source event: entry-93
- [x] `ac7_4` docs - Install section lists Go, npm, PyPI, GitHub release, and OS package options separately.
  - Command: `for pattern in 'GitHub release binary' '^Go:$' '^Homebrew:$' '^npm:$' '^PyPI:$' '^Scoop:$' 'WinGet'; do rg -n "$pattern" README.md docs/installation.md >/dev/null || exit 1; done`
  - Expected kind: `exit_code_zero`
  - Status: pass
  - Evidence: exit code was 0
...
Acceptance:
- [x] `ac8_1` lifecycle - Full local check passes from a clean tree.
  - Command: `make check`
  - Expected kind: `exit_code_zero`
  - Status: pass
  - Evidence: exit code was 0
  - Source event: entry-107
- [x] `ac8_2` release - Snapshot release builds from the release candidate.
  - Command: `make release-snapshot`
  - Expected kind: `exit_code_zero`
  - Status: pass
  - Evidence: exit code was 0
  - Source event: entry-108
- [x] `ac8_3` dogfood - This spec has passed hardening before approval.
  - Command: `go run ./cmd/scafld status scafld-product-hardening-followup`
  - Expected kind: `exit_code_zero`
  - Status: pass
  - Evidence: exit code was 0
  - Source event: entry-109
- [x] `ac8_4` tool - External adversarial review provider is available for the review gate.
  - Command: `command -v codex >/dev/null || command -v claude >/dev/null`
  - Expected kind: `exit_code_zero`
  - Status: pass
  - Evidence: exit code was 0
  - Source event: entry-110
- [x] `ac8_5` smoke - Local release installers verify against rebuilt GitHub-style artifacts.
  - Command: `scripts/smoke-release-installers.sh 0.0.0-snapshot`
  - Expected kind: `exit_code_zero`
  - Status: pass
  - Evidence: exit code was 0
...
Summary: No open blockers found in static verification of the repair. The two prior blockers appear addressed: completion now rejects stale spec/workspace seals, and sentinel reviewed_head values no longer authorize external reviews. Residual risk: tests could not be executed in this read-only sandbox.

Attack log:
- `git status/diff`: Workspace inventory -> clean (Reviewed current dirty set and diff stat. Changes are concentrated in review sealing, completion authority, provider provenance, docs, and related tests.)
- `internal/app/complete, internal/core/completion, internal/adapters/cli/helpers.go`: Completion gate trace -> clean (Verified CLI complete now passes git.Adapter into complete.Run, and complete checks session packet/hash plus current spec/workspace seals before completion.)
- `internal/app/review`: Review persistence trace -> clean (Verified review records review_packet, canonical_response_sha256, provider model/session, reviewed head/dirty/diff, and reviewed_spec after provider execution and mutation checking.)
- `internal/core/spec/model.go, internal/app/review/scope.go, internal/app/complete/complete.go`: Prior blocker verification: stale spec edits -> clean (Active task spec edits are excluded from workspace diff but covered by spec.ContractDigest; mutation during review is still blocked via currentSpecReviewPath.)
- `internal/core/completion/authority.go`: Prior blocker verification: sentinel reviewed_head -> clean (Completion authority now rejects external review entries whose reviewed_head is empty, unavailable, or error-prefixed; tests cover unavailable and error sentinels.)
- `internal/adapters/providers/clients.go, internal/adapters/providers/invoke.go`: Provider provenance trace -> clean (Codex now stamps the configured model into AgentResponse; Claude provenance remains extracted from system.init and copied into dossiers/session entries.)
- `docs/review.md, docs/run-artifacts.md`: Docs and artifact contract -> clean (Docs were updated to describe sealed review packets, canonical hashes, provider provenance, and complete-gate freshness checks.)
- `test execution`: Focused tests -> skipped (Attempted `go test ./internal/app/complete ./internal/app/review ./internal/core/completion ./internal/adapters/providers`, but the read-only sandbox blocked Go from creating its build work directory under /var/folders.)
