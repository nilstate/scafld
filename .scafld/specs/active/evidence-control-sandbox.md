...
- Resolve the reviewer binary from a fixed authoritative path for receipt-grade gate review, hash its bytes, and surface `binary_sha256` for the receipt's `reviewer.binary_sha256`.
...
- Disable provider memory autoload on the gate path and blocklist CLAUDE.md, AGENTS.md (recursively), .scafld/config.yaml, and GEMINI.md from ever being pinned as evidence.
...
- Ignore config.local for receipt-grade gate reviewer selection so the host cannot reconfigure its own challenger; ordinary `scafld review` keeps the existing config.local overlay.
...
- In scope: committed base-config endpoint pin fields for provider configs, read without config.local on receipt-grade gate review.
...
- In scope: ignoring config.local for receipt-grade gate reviewer selection (internal/adapters/config/config.go reviewer-selection read).
...
- Out of scope: the `scafld_gate` verb, acceptance folding (runFinalAcceptance), receipt assembly, and ed25519 signing; that is owned by spec host-gate-and-receipt. This spec only produces the runtime facts (`binary_sha256`, `endpoint_host`, evidence provenance) the receipt later records.
...
- Real repo: config overlay merges config.local.yaml today via `Load`/`overlay` in internal/adapters/config/config.go; the gate path must read base config only for reviewer selection.
...
- A fixed reviewer-binary path is available via existing provider config (`ProviderConfig.Binary` / `ExternalReviewConfig.ProviderBinary` in internal/adapters/config/config.go). For receipt-grade gate review it is authoritative: if absent, non-absolute, not executable, or hash-unreadable, the gate fails closed. PATH lookup is permitted only on non-receipt-grade smoke paths.
...
- Endpoint host is derived from pinned config or a built-in provider default after env scrub; host env endpoint vars never define it on the receipt-grade gate path.
...
- Receipt-grade activation is explicit. `providers.Select`, `providers.SelectHarden`, ordinary `Provider.Invoke`, and ordinary `scafld review` keep current behavior. Receipt-grade gate review uses a distinct API:
...
- Config-local skipping applies only to receipt-grade gate reviewer selection. Ordinary `scafld review`, `scafld harden`, and local smoke runs retain the existing overlaid config behavior.
...
- internal/core/reviewevidence/evidence.go (new: EvidenceFile, Provenance, status/path validation shared by fingerprint/gate/evidence sandbox)
...
- internal/adapters/providers/provider.go - Reviewer providers point CWD and read-tool jail at the scratch dir from the sandbox builder, and set provider memory autoload off (Claude/Gemini settings, Codex `--ignore-user-config` already present) consistently on the gate path.
...
Objective: Ensure the host agent cannot reconfigure its own challenger by editing config.local. On the receipt-grade gate path, reviewer selection reads committed base config only, skipping the config.local.yaml overlay that `Load`/`overlay` applies today. This is a narrow, gate-path-only read; ordinary `scafld review`, `scafld harden`, and local smoke paths keep the full overlay.

Changes:
- internal/adapters/config/config.go - Add a receipt-grade gate config read that loads base config without the config.local.yaml overlay for reviewer selection and endpoint pins, reusing the existing `readConfigFile`/`withDefaults` machinery and leaving the general `Load` overlay unchanged.
...
- Fences cleanly against neighbours: tree_sha (commit-free-tree-fingerprint), reviewer selection/independence (context-isolated-reviewer), and gate verb/receipt/signing (host-gate-and-receipt).
...
- [high/blocks approval] `harden-1` question - The env scrub objective conflicts with existing `Request.Env` semantics and the promise to preserve acceptance-command behavior.
  ...
  - Recommended answer: Add an `InheritEnv`/`EnvMode` field to `execution.Request`; default preserves current inheritance, reviewer gate requests use exact env.
  - If unanswered: Default to an explicit `InheritEnv` or `EnvMode` on `execution.Request`, with current inherit behavior as the default and reviewer gate requests opting into exact env.
- [high/blocks approval] `harden-2` question - The scratch-dir builder cannot be implemented without inventing how canonical file bytes reach providers.
  ...
  - Recommendation: Define the handoff type and owner now: either extend the gate/review request with canonical evidence entries or add a narrow provider sandbox input assembled by the future gate use case.
  - Question: What concrete interface supplies canonical reviewed bytes to the evidence sandbox builder?
  - Recommended answer: Introduce a canonical evidence input type with path, status, sha256, and bytes, produced by the fingerprint/gate path and consumed by `evidence_sandbox.go`.
...
- [high/blocks approval] `harden-3` question - Fixed-path binary pinning is under-specified and currently conflicts with PATH-based auto discovery.
  ...
  - Evidence: Provider auto availability currently accepts explicit binaries or falls back to `osexec.LookPath(name)`. The draft requires resolving reviewer binaries from a fixed configured path rather than host PATH, but reviewer selection is declared out of scope and the spec does not say whether `provider:auto`, `--provider-binary`, per-provider config binaries, or `--provider command` are allowed on the gate path.
  ...
  - Question: What exact binary source is authoritative on the gate path for each provider and for `provider:auto`?
  ...
  - If unanswered: Default to fail-closed on the gate path unless the selected provider has an absolute configured binary path; do not use `LookPath` or shell command providers for gate receipts.
...
- [low/advisory] `harden-6` question - The config.local bypass needs a precise command boundary, but this can be resolved during implementation.
  ...
  - Evidence: CLI review selection currently calls `configadapter.Load`, which overlays `.scafld/config.local.yaml` before provider selection. The planned phase 3 touchpoint is valid, but the spec says config.local should be ignored on the gate path; current `scafld review` may remain non-gate lifecycle behavior depending on the future gate split.
  ...
  - Question: Does phase 3 change only the future receipt gate path, or the existing `scafld review` command too?
  - Recommended answer: Only the receipt-grade gate reviewer selection ignores config.local; ordinary lifecycle `scafld review` keeps current local overlay unless it is promoted to the gate path.
  - If unanswered: Default to applying base-only config only to receipt-grade gate reviewer selection, while documenting whether regular `scafld review` keeps local overlays.
...
- scope/migration audit
  ...
  - Result: failed
  - Evidence: providers.Select and providers.SelectHarden both construct the same provider implementations, and InvokeAgent paths are used for review/harden without a gate-only discriminator. Ordinary review selection is built in internal/adapters/cli/review/selection.go from configadapter.Load.
- acceptance timing audit
  - Grounded in: spec_gap:phases
  - Result: failed
...
- design challenge
  ...
  - Result: failed
  - Evidence: The product goal is valid: reviewer independence is weak while providers inherit host env, PATH, live CWD, and unfenced changed content. The design needs explicit gate-only APIs to avoid hardening ordinary review/harden by accident or discarding receipt facts before host-gate consumes them.

Issues:
- [high/blocks approval] `harden-1` question - Receipt-grade-only behavior has no concrete activation boundary.
  - Status: open
  - Grounded in: code:internal/adapters/providers/provider.go:97
  - Evidence: The current provider constructors are shared by review and harden: providers.Select starts at provider.go:97, providers.SelectHarden is used by internal/adapters/cli/harden/selection.go:105, and normal review uses configadapter.Load plus providers.Select at internal/adapters/cli/review/selection.go:46 and :66. The draft says exact env, pinned binary, scratch CWD, and config.local-ignore apply only to receipt-grade gate review, but it does not define a Gate/ReceiptGrade flag, separate selector, or separate invocation path.
  - Recommendation: Specify a gate-only discriminator in the provider/config/review path and require tests for both branches: receipt-grade exact sandboxed execution and ordinary inherited-env review/harden.
...
- [high/blocks approval] `harden-2` question - Receipt runtime facts are specified but not connected to a durable caller-visible contract.
  ...
  - Recommended answer: Expose a typed RuntimeFacts/EvidenceProvenance field on the gate-only provider result consumed by host-gate, not only on transient AgentResponse.
...
- [high/blocks approval] `harden-4` question - Evidence sandbox behavior is too broad to implement without invention.
  ...
  - Recommended answer: Make evidence_sandbox.go produce a typed Sandbox{CWD, ReadRoots, Env, ArgsPolicy, Provenance, Cleanup} consumed by gate-only providers.
