...
echo "[1/7] init workspace and baseline commit"
...
echo "[2/7] branch --json creates a task branch and records origin metadata"
...
echo "[3/7] status --json surfaces origin and sync directly"
...
echo "[4/7] branch refuses to switch when the engineering worktree is dirty"
printf 'dirty\n' >> "$WS/tracked.txt"
...
echo "[5/7] sync --json reports drift for uncommitted engineering changes"
...
echo "[6/7] sync detects branch mismatch once the engineering tree is clean"
...
echo "[7/7] branch --bind-current rebases the spec onto an existing manual branch"
