...
class CommandRuntimeTest(unittest.TestCase):
    def test_find_root_returns_nearest_workspace(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (workspace / ".scafld").mkdir(parents=True)
            (workspace / ".scafld" / "config.yaml").write_text("modes: {}\n", encoding="utf-8")
    ...
    def test_command_context_uses_discovered_root(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (workspace / ".scafld" / "core").mkdir(parents=True)
            (workspace / ".scafld" / "core" / "manifest.json").write_text("{}", encoding="utf-8")
            nested.mkdir(parents=True)
