...
python3 - <<PY || fail "plan did not open harden on the new draft"
import yaml
...
python3 - <<PY || fail "harden did not append a new round"
import yaml
d = yaml.safe_load(open("$WS/.ai/specs/drafts/t1.yaml"))
assert d.get("harden_status") == "in_progress", d.get("harden_status")
...
python3 - <<PY || fail "could not clear harden rounds"
import yaml
p = "$WS/.ai/specs/drafts/t4.yaml"
d = yaml.safe_load(open(p))
