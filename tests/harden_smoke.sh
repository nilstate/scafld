...
[ -d "$WS/.scafld/specs/drafts" ] || fail "init did not create drafts/"

echo "[2/13] scafld plan creates a draft and opens harden"
run_scafld plan t1 -t 'test one' -s small -r low >/dev/null || fail "scafld plan t1 failed"
PYTHONPATH="$REPO_ROOT" python3 - <<PY || fail "plan did not open harden on the new draft"
from pathlib import Path
from scafld.spec_markdown import parse_spec_markdown
d = parse_spec_markdown(Path("$WS/.scafld/specs/drafts/t1.md").read_text())
assert d.get("harden_status") == "in_progress", d.get("harden_status")
assert len(d.get("harden_rounds") or []) == 1, d.get("harden_rounds")
PY

echo "[3/13] scafld harden prints HARDEN MODE and appends a round"
output=$(run_scafld harden t1)
[[ "$output" == *"HARDEN MODE"* ]] || fail "harden prompt missing HARDEN MODE header"
PYTHONPATH="$REPO_ROOT" python3 - <<PY || fail "harden did not append a new round"
from pathlib import Path
from scafld.spec_markdown import parse_spec_markdown
d = parse_spec_markdown(Path("$WS/.scafld/specs/drafts/t1.md").read_text())
assert d.get("harden_status") == "in_progress", d.get("harden_status")
assert len(d.get("harden_rounds") or []) == 2, d.get("harden_rounds")
PY

echo "[4/13] scafld harden --mark-passed closes the round"
run_scafld harden t1 --mark-passed >/dev/null || fail "mark-passed failed"
PYTHONPATH="$REPO_ROOT" python3 - <<PY || fail "mark-passed did not set passed / close round"
from pathlib import Path
from scafld.spec_markdown import parse_spec_markdown
d = parse_spec_markdown(Path("$WS/.scafld/specs/drafts/t1.md").read_text())
...
PYTHONPATH="$REPO_ROOT" python3 - <<PY || fail "re-run did not append round or reset status"
from pathlib import Path
from scafld.spec_markdown import parse_spec_markdown
d = parse_spec_markdown(Path("$WS/.scafld/specs/drafts/t1.md").read_text())
assert d.get("harden_status") == "in_progress", d.get("harden_status")
assert len(d["harden_rounds"]) == 3, d["harden_rounds"]
PY
...
assert_json "$output" "data['state']['harden_status'] == 'in_progress' and data['state']['round'] == 4" "harden --json should emit round and status"
...
assert_json "$output" "data['state']['harden_status'] == 'passed' and data['state']['round'] == 4" "mark-passed --json should emit passed status"
...
SCAFLD_SPEC_CREATED="2026-04-20T02:00:00Z" \
  write_markdown_spec "$WS/.scafld/specs/drafts/t2.md" "t2" "draft" "Fixture" "none" "true"
...
SCAFLD_SPEC_CREATED="2026-04-20T02:00:00Z" \
  write_markdown_spec "$WS/.scafld/specs/drafts/t3.md" "t3" "draft" "Fixture" "none" "true"
perl -0pi -e 's/^harden_status:.*\n//m' "$WS/.scafld/specs/drafts/t3.md"
...
run_scafld plan t5 -t 'valid citations' -s small -r low >/dev/null || fail "scafld plan t5 failed"
mkdir -p "$WS/.scafld/specs/archive/2026-04"
SCAFLD_SPEC_CREATED="2026-04-20T02:00:00Z" \
SCAFLD_SPEC_PHASE_STATUS="completed" \
  write_markdown_spec "$WS/.scafld/specs/archive/2026-04/archived-ok.md" "archived-ok" "completed" "Archived fixture" "AGENTS.md" "true"
PYTHONPATH="$REPO_ROOT" python3 - <<PY || fail "could not write valid harden citations"
from pathlib import Path
from scafld.spec_markdown import parse_spec_markdown, update_spec_markdown
p = "$WS/.scafld/specs/drafts/t5.md"
path = Path(p)
text = path.read_text()
d = parse_spec_markdown(text)
...
path.write_text(update_spec_markdown(text, d))
PY
...
run_scafld plan t6 -t 'invalid citations' -s small -r low >/dev/null || fail "scafld plan t6 failed"
PYTHONPATH="$REPO_ROOT" python3 - <<PY || fail "could not write invalid harden citations"
from pathlib import Path
from scafld.spec_markdown import parse_spec_markdown, update_spec_markdown
p = "$WS/.scafld/specs/drafts/t6.md"
path = Path(p)
text = path.read_text()
d = parse_spec_markdown(text)
d["harden_rounds"][-1]["questions"] = [
...
run_scafld plan t4 -t 'empty rounds' -s small -r low >/dev/null || fail "scafld plan t4 failed"
PYTHONPATH="$REPO_ROOT" python3 - <<PY || fail "could not clear harden rounds"
from pathlib import Path
from scafld.spec_markdown import parse_spec_markdown, update_spec_markdown
p = "$WS/.scafld/specs/drafts/t4.md"
path = Path(p)
text = path.read_text()
d = parse_spec_markdown(text)
d["harden_status"] = "not_run"
d["harden_rounds"] = []
path.write_text(update_spec_markdown(text, d))
PY
set +e
