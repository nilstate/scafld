...
from scafld.config import (
    detect_init_config,
    load_config as load_workspace_config,
    parse_yaml_value,
    render_init_local_config,
)
from scafld.command_runtime import find_root, find_workspaces, require_root
...
from scafld.projections import (
    build_projection_model,
    humanize_binding_mode,
    origin_payload,
    phase_counts,
    summarize_origin_source,
)
...
def cmd_init(args):
    ...
    for d in [DRAFTS_DIR, APPROVED_DIR, ACTIVE_DIR, ARCHIVE_DIR, LOGS_DIR,
        ...
        result["directories"].append({"path": f"{d}/", "action": "ok"})
        if not json_mode:
    ...
    # Sync the managed bundle after the legacy overlay exists so repeated
    # `scafld init` runs do not churn the manifest on already-initialized repos.
    if not json_mode:
...
def cmd_audit(args):
    ...
    if not declared:
        ...
        if json_mode:
            emit_command_json(
                "audit",
                task_id=args.task_id,
                state={"mode": "working tree" if not args.base else f"vs {args.base}"},
                result={
                    "declared": [],
                    "matched": [],
                    "undeclared": [],
    ...
    if args.base:
        ...
        if actual is None:
            raise ScafldError(
                f"could not diff against base ref {args.base!r}",
                [error] if error else [],
            )
    else:
    ...
    if matched:
        ...
        for f in sorted(matched):
            suffix = f" [{declared_changes[f]}]" if declared_changes.get(f) != "exclusive" else ""
            print(f"    {c(C_GREEN, '✓')} {f}{suffix}")
    ...
    if covered_by_other_active:
        ...
        for f in sorted(covered_by_other_active):
            owners = describe_other_active_specs(f, covered_by_other_active_details)
            suffix = f" ({owners})" if owners else ""
            print(f"    {c(C_CYAN, '↷')} {f}{suffix}")

    if shared_with_other_active:
        print(f"\n  {c(C_CYAN, 'Shared with other active specs')} ({len(shared_with_other_active)}):")
        for f in sorted(shared_with_other_active):
            owners = describe_other_active_specs(f, shared_details)
            suffix = f" ({owners})" if owners else ""
            print(f"    {c(C_CYAN, '≈')} {f}{suffix}")

    if active_overlap:
        print(f"\n  {c(C_RED, 'Conflict - exclusive overlap with other active specs')} ({len(active_overlap)}):")
        for f in sorted(active_overlap):
            owners = describe_other_active_specs(f, conflict_details)
            suffix = f" ({owners})" if owners else ""
            print(f"    {c(C_RED, '×')} {f}{suffix}")
