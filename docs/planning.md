...
When a spec is finished but you want to stress-test it before approval, run `scafld harden <task-id>`. This enters HARDEN MODE: the agent audits paths, commands, scope and migration claims, acceptance timing, rollback or repair shape, and the core design before asking questions. The design challenge asks why the plan exists, what underlying product or system problem it solves, and whether the proposed change is a short-sighted bandaid over an endemic issue. If a question can be answered by exploring the codebase, the agent should inspect the code instead of asking. Harden records approval blockers and advisories as issues; only open issues with `blocks_approval` force another round. When the contract is sharp enough to execute, run `scafld harden <task-id> --mark-passed`. The round is recorded in `harden_rounds` for audit.
